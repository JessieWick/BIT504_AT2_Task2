...
public class Board {
	...
	public Board() {
	 ...
	 //TODO: initialise the cells array using ROWS and COLS constants 
		cells = new Cell[3][3]; //initialising cell array
	...
	public boolean isDraw() {
		...
		for (int row =0; row < GameMain.ROWS; ++row) {
			...
			for (int col = 0; col < GameMain.COLS; ++col)		
			{
				if (cells [row][col].content == Player.Empty) {		//returning a true or false statement for 3 in a row
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		if(cells[0][playerRow].content == thePlayer && cells[1][playerRow].content == thePlayer && cells[2][playerRow].content == thePlayer)
			return true;
